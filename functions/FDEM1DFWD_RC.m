...
%
S.r= sqrt(S.x^2 + S.y^2 + (S.z+S.height)^2);                               % Coil spacing (m) 收发距
S.omega= 2*pi*S.freq;                                                      % Angular frequency (Rad/s) 角频率
%
%% Get magnetic fields
%
...
%% Variables
%
mu0=    4*pi*1e-7;                                                         % Vacuum permeability (N/A) 真空磁导率
eps0=   1/(35950207149.4727056*pi);                                        % Vacuum permittivity (F/m) 真空介电常数
%
...
lambda = l1' .* l2';
%
%% Evaluate function at lambda：  K（λ）
%
...
%% Calculate u0 and u
%
k0= S.omega.^2.*mu0.*eps0-1i*S.omega.*mu0.*sigma0;
u0= sqrt(lambda.^2 - k0);
